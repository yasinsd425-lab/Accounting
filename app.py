...
<style>
    ...
    /* Premium Smooth Dark Abstract Background */
    .stApp {
    ...
    /* Sidebar styling override for Glassmorphism */
    ...
    h1, h2, h3, h4, h5, h6, label, .stMetric label {
        color: #f1f5f9 !important;
        font-weight: 600 !important;
    }
    
    /* Dataframe Styling Compatibility */
...
# ==========================================
def init_db():
    conn = sqlite3.connect('finance_v3.db', check_same_thread=False)
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS income (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, source TEXT, amount REAL, inc_type TEXT, timestamp TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS fixed_expenses (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, name TEXT, amount REAL, month INTEGER, year INTEGER, timestamp TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS variable_expenses (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, category TEXT, description TEXT, amount REAL, month INTEGER, year INTEGER, timestamp TEXT)''')
    conn.commit()
    return conn
...
# Initialize Form Persistence Keys (For 100% reliable clearing)
...
def get_monthly_income():
    cursor.execute('SELECT amount, inc_type FROM income WHERE user_id = ?', (user_id,))
...
def get_total_expenses(m, y):
    cursor.execute('SELECT SUM(amount) FROM fixed_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, m, y))
    fixed = cursor.fetchone()[0] or 0.0
    cursor.execute('SELECT SUM(amount) FROM variable_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, m, y))
    var = cursor.fetchone()[0] or 0.0
    return fixed + var
...
with tab_dash:
    ...
    # Grid KPIs: 2 Rows to avoid text squeezing
    ...
    # Custom Gradient-like Color Progress Bar
    if m_income > 0:
    ...
    cursor.execute('SELECT name, amount FROM fixed_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, sel_month, sel_year))
    f_data = cursor.fetchall()
    cursor.execute('SELECT category, SUM(amount) FROM variable_expenses WHERE user_id = ? AND month = ? AND year = ? GROUP BY category', (user_id, sel_month, sel_year))
    v_data = cursor.fetchall()
    
    chart_data = [{'Segment': row[0], 'Amount': row[1]} for row in f_data] + \
                 [{'Segment': row[0], 'Amount': row[1]} for row in v_data]
    ...
    # Bottom Section: Stacked Granular Tables (Full Width to avoid clutter)
    ...
    st.markdown("<br>", unsafe_allow_html=True) # Adding empty space between sections
        
    # Variable Expenses Breakdown
    st.markdown("#### 🔹 Variable Expenses Breakdown")
    cursor.execute('SELECT category, description, amount FROM variable_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, sel_month, sel_year))
...
with tab_inc:
    ...
    with st.container():
        # Using a dynamic key to force clear on submit
        with st.form(f"add_income_pro_{st.session_state.inc_key}"):
            ...
            if st.form_submit_button("Inject Income Asset"):
                if i_src and i_amt > 0:
                    cursor.execute('INSERT INTO income (user_id, source, amount, inc_type, timestamp) VALUES (?, ?, ?, ?, ?)',
                                   (user_id, i_src, i_amt, i_type, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    conn.commit()
                    st.success(f"Successfully committed {i_type} asset vector.")
                    # Change form key to destroy old form and spawn a fresh one (clears data)
                    st.session_state.inc_key += 1
                    st.rerun()

    st.markdown("### Active Income Infrastructure Asset Matrix")
    cursor.execute('SELECT id, source, amount, inc_type FROM income WHERE user_id = ?', (user_id,))
    ...
    if raw_inc:
        ...
        total_row = pd.DataFrame([[
            'TOTALS', '', '', 0.0, 
            df_inc_matrix['Annual Equiv (€)'].sum(), 
            df_inc_matrix['Monthly Equiv (€)'].sum()
        ]], columns=['Asset ID', 'Source Vector', 'Horizon Type', 'Stated Amount (€)', 'Annual Equiv (€)', 'Monthly Equiv (€)'])
        ...
        with st.expander("✏️ Administrative Modifications & Asset Liquidation"):
            ...
            if col_action.button("Commit Strategic Update", key='btn_inc_upd', use_container_width=True):
                cursor.execute('UPDATE income SET source=?, amount=? WHERE id=?', (up_src, up_amt, target_id))
                conn.commit()
                st.rerun()
            if col_action.button("Liquidate/Delete Asset", key='btn_inc_del', type="primary", use_container_width=True):
                cursor.execute('DELETE FROM income WHERE id=?', (target_id,))
                conn.commit()
                st.rerun()
    else:
...
with tab_fix:
        ...
        with st.form(f"add_fixed_debit_{st.session_state.fix_key}"):
            ...
            if st.form_submit_button("Commit Fixed Structural Entry"):
                if fixed_title and fixed_val > 0:
                    cursor.execute('INSERT INTO fixed_expenses (user_id, name, amount, month, year, timestamp) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    cursor.execute('SELECT id, name, amount, timestamp FROM fixed_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, sel_month, sel_year))
    ...
    if raw_fix:
        ...
        total_fix_row = pd.DataFrame([[
            'TOTALS', '', 
            df_fix_matrix['Absolute Cost (€)'].sum(), ''
        ]], columns=['Debit ID', 'Identifier Label', 'Absolute Cost (€)', 'Temporal Timestamp'])
        ...
        with st.expander("✏️ Structural Adjustments & Debit Deletion"):
            ...
            if col_act.button("Modify Entry Structure", key='btn_fix_upd', use_container_width=True):
                cursor.execute('UPDATE fixed_expenses SET name=?, amount=? WHERE id=?', (up_f_lbl, up_f_val, target_f_id))
                conn.commit()
                st.rerun()
            if col_act.button("Wipe Entry Record", key='btn_fix_del', type="primary", use_container_width=True):
                cursor.execute('DELETE FROM fixed_expenses WHERE id=?', (target_f_id,))
                conn.commit()
                st.rerun()
    else:
...
with tab_var:
        ...
        with st.form(f"add_var_debit_{st.session_state.var_key}"):
            ...
            if st.form_submit_button("Commit Transaction Entry"):
                if v_amount_field > 0 and v_description_field:
                    cursor.execute('INSERT INTO variable_expenses (user_id, category, description, amount, month, year, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)',
    ...
    cursor.execute('SELECT id, category, description, amount, timestamp FROM variable_expenses WHERE user_id = ? AND month = ? AND year = ?', (user_id, sel_month, sel_year))
    ...
    if raw_var:
        ...
        for current_segment in df_var_matrix['Segment Category'].unique():
            ...
            subtotal_row = pd.DataFrame([[
                'SEGMENT TOTAL', '', 
                df_segmented_view['Absolute Value (€)'].sum(), ''
            ]], columns=['Transaction ID', 'Description Ledger', 'Absolute Value (€)', 'Timestamp'])
        ...
        with st.expander("✏️ Dynamic Ledger Refactoring Controls"):
            ...
            if col_act.button("Modify Matrix Entry", key='btn_var_upd', use_container_width=True):
                cursor.execute('UPDATE variable_expenses SET category=?, description=?, amount=? WHERE id=?', (up_v_cat, up_v_dsc, up_v_amt, target_v_id))
                conn.commit()
                st.rerun()
            if col_act.button("Purge Entry Completely", key='btn_var_del', type="primary", use_container_width=True):
                cursor.execute('DELETE FROM variable_expenses WHERE id=?', (target_v_id,))
                conn.commit()
                st.rerun()
    else:
